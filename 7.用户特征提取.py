...
t = t.drop_duplicates()
t['user_click_difshop_total']=1 #用户点击商家数
...
'user_id']].to_csv('newdata/1/user_feature1.csv',index=None)
...
t = t.groupby(['user_gender_id','user_occupation_i d','user_star_level','user_age_level']).agg('sum').reset_index()
t = t.rename(columns={'is_trade':'gosa_buy'})
tra = pd.merge(tra,t,on=['user_gender_id','user_occupation_id','user_star_level','user_age_level'],how='left')
# lv
tra['gosa_rate'] = tra['gosa_buy']/tra['gosa_click']
tra[['gosa_click','gosa_buy','gosa_rate','user_gender_id','user_occupation_id','user_star_level','user_age_level']].to_csv('newdata/1/gosa_feature1.csv',index=None)
