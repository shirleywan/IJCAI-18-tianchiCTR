...
# 提取训练集一广告商品相关特征
train1_f = pd.read_csv('data/train1_f.csv') #这里要将新的值存在新的文件中，所以不是直接在train1_f中操作
tra = train1_f
# 该展示商品被点击次数
t = tra[['item_id']]
t['item_click']=1#新增类，用来记录个数，这样写即可
t = t.groupby('item_id').agg('sum').reset_index()#每个商品被点击次数
tra = pd.merge(tra,t,on='item_id',how='left')
# 该展示商品被购买次数
t = tra[['item_id','is_trade']]
t = t.groupby('item_id').agg('sum').reset_index()#根据商品id，把is_trade加和，也就是该商品被购买次数
t = t.rename(columns={'is_trade':'item_buy'})#重命名为item_buy
tra = pd.merge(tra,t,on='item_id',how='left')
# 该展示商品被购买率
tra['item_buy_rate'] = tra['item_buy']/tra['item_click']
# 该展示商品在上午购买的次数
t = tra[['item_id','is_am','is_trade']]
t = t[t.is_trade==1][['item_id','is_am']]#提取出满足条件的记录，is_am是1和0的形式，求和即为上午购买的商品数量
...
t['sale_to_occupation'] = 1#这里标记为1的是item_id和不同职业，加和后用来记录每个商品被多少种职业购买
...
t = tra.groupby('cate2').agg('sum').reset_index()#该类别商品被购买过多少次
t['is_high_sale'] = t['is_trade'].apply(lambda x:1 if x>=1000 else 0) #高销售商品的阈值定为1000
...
def getPropertySet(s):
     ...
     return PropertySet
t['item_property_list']=t.item_property_list.apply(getPropertySet) #把prop类的string转为list
#获取受欢迎商品的属性
t1=t[t.is_trade==1]
Popular=set()
...
def setSum_propertys(s):
      return len(s)
t2['Sum_Propertys']=t2.item_property_list.apply(setSum_propertys)#每个商品属于多少个prop中
#记录每个商品的受欢迎属性个数
...
t2['Sum_Popular_Propertys']=t2.item_property_list.apply(getSum_Popular_Propertys)#每个商品的列表在popular受欢迎的set中的个数
...
t = t[t.is_trade==1]
t = t.drop_duplicates(subset=['item_id','user_id'])#这里对每个用户有去重
t = t.groupby('item_id')['is_trade'].agg('sum').reset_index()#每个商品被多少个用户购买
...
tra[['item_id','sale_on_pm','item_click','item_buy','item_buy_rate','sale_on_am','is_high_sale','Sum_Propertys','Sum_Popular_Propertys','Popular_rate'
     ]].to_csv('newdata/1/item_feature1.csv',index=None)
