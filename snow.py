...
def base_process(data):#基础预处理
    ...
    for i in range(1, 3):
        data['item_category_list' + str(i)] = lbl.fit_transform(data['item_category_list'].map(
            lambda x: str(str(x).split(';')[i]) if len(str(x).split(';')) > i else ''))  # item_category_list 的第 0 列全部都一样
    for i in range(10):
    ...
    # data['item_property_list'] = count_vec.fit_transform(data['item_property_list'])
    # # 既然采用自然语言处理技术提取的属性，
    # # 那我们就用文本挖掘中的向量空间模型处理 item_property_list。注意，predict_category_property 也需要进行相应处理。
...
def slide_cnt(data):
    ...
    print('当前日期前一天的 cnt')
    for d in range(19, 26):  # 18 到 24 号
        df1 = data[data['day'] == d - 1]
        df2 = data[data['day'] == d]  # 19 到 25 号
...
def item(data):
    print('一个 item 有多少 brand,price salse collected level……')
    ...
    print('一个 brand 有多少 price salse collected level……')
    ...
    print('一个 city 有多少 item_price_level，item_sales_level，item_collected_level，item_pv_level')
    ...
    print('一个 price 有多少 item_sales_level，item_collected_level，item_pv_level')
    ...
    print('一个 item_sales_level 有多少 item_collected_level，item_pv_level')
    ...
    print('一个 item_collected_level 有多少 item_pv_level')
...
def user(data):
    ...
    print('user_age_level 对应的 user_occupation_id，user_star_level')
    ...
    print('user_occupation_id 对应的 user_star_level')
...
def user_item(data):
    itemcnt = data.groupby(['user_id'], as_index=False)['instance_id'].agg({'user_cnt': 'count'})
    data = pd.merge(data, itemcnt, on=['user_id'], how='left')
    print('一个 user 有多少 item_id,item_brand_id……')
    for col in ['item_id',
    ...
    print('一个 user_gender 有多少 item_id,item_brand_id……')
    itemcnt = data.groupby(['user_gender_id'], as_index=False)['instance_id'].agg({'user_gender_cnt': 'count'})
    data = pd.merge(data, itemcnt, on=['user_gender_id'], how='left')
    for col in ['item_id',
                'item_brand_id','item_city_id','item_price_level',
    ...
    print('一个 user_age_level 有多少 item_id,item_brand_id……')
    itemcnt = data.groupby(['user_age_level'], as_index=False)['instance_id'].agg({'user_age_cnt': 'count'})
    data = pd.merge(data, itemcnt, on=['user_age_level'], how='left')
    for col in ['item_id',
                'item_brand_id','item_city_id','item_price_level',
    ...
    print('一个 user_occupation_id 有多少 item_id,item_brand_id…')
    itemcnt = data.groupby(['user_occupation_id'], as_index=False)['instance_id'].agg({'user_occ_cnt': 'count'})
    data = pd.merge(data, itemcnt, on=['user_occupation_id'], how='left')
    for col in ['item_id',
                'item_brand_id','item_city_id','item_price_level',
...
def user_shop(data):
    print('一个 user 有多少 shop_id,shop_review_num_level……')
    ...
    print('一个 user_gender 有多少 shop_id,shop_review_num_level……')
    for col in ['shop_id', 'shop_review_num_level', 'shop_star_level']:
    ...
    print('一个 user_age_level 有多少 shop_id,shop_review_num_level……')
    for col in ['shop_id', 'shop_review_num_level', 'shop_star_level']:
    ...
    print('一个 user_occupation_id 有多少 shop_id,shop_review_num_level……')
    for col in ['shop_id', 'shop_review_num_level', 'shop_star_level']:
...
def shop_item(data):
    print('一个 shop 有多少 item_id,item_brand_id,item_city_id,item_price_level……')
    ...
    print('一个 shop_review_num_level 有多少 item_id,item_brand_id,item_city_id,item_price_level……')
    ...
    # print('一个 shop_star_level 有多少 item_id,item_brand_id,item_city_id,item_price_level……')
...
if __name__ == "__main__":
    # train = pd.read_csv("E:/Alimama-master11111/Alimama-master/newdata10/train2aa.txt", sep="\s+")
    # test = pd.read_csv("E:/Alimama-master11111/Alimama-master/newdata10/round2_test.txt", sep="\s+")
    train = pd.read_csv("data/train.txt", sep = '\s+')
    test = pd.read_csv("data/test.txt", sep='\s+')
    data = pd.concat([train, test])
    data = data.drop_duplicates(subset='instance_id')  # 把 instance id 去重
    print('make feature')
