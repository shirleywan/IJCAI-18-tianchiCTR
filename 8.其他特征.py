...
train[['before','after','now','user_id','item_id','hour']].to_csv('data/other_user_item_hour_feature1.csv',index=None)
...
train[['ubefore','uafter','unow','user_id','unow_rate','hour']].to_csv('data/other_user_hour_feature1.csv',index=None)
...
t = t.drop_duplicates(subset=['user_id','item_id','hour'])
train = pd.merge(train,t,on=['user_id','item_id','hour'],how='left')
train[['before','after','now','user_id','item_id','hour']].to_csv('data/other_user_item_hour_feature2.csv',index=None)
...
train = pd.merge(train,t,on=['user_id','hour'],how='left')
train['unow_rate'] = train['unow']/train['user_today_click']
train[['ubefore','uafter','unow','unow_rate','user_id','hour']].to_csv('data/other_user_hour_feature2.csv',index=None)
...
t = t.drop_duplicates(subset=['user_id','item_id','hour'])
train = pd.merge(train,t,on=['user_id','item_id','hour'],how='left')
train[['before','after','now','user_id','item_id','hour']].to_csv('data/other_user_item_hour_feature3.csv',index=None)
...
train = pd.merge(train,t,on=['user_id','hour'],how='left')
train['unow_rate'] = train['unow']/train['user_today_click']
train[['ubefore','uafter','unow','user_id','unow_rate','hour']].to_csv('data/other_user_hour_feature3.csv',index=None)
# 商户时间提取
t = train[['shop_id','hour']]
# 该商户在该时段被点击次数
t['shop_hour_tclick'] = 1
